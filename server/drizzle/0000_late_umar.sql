...
--> statement-breakpoint
CREATE TABLE `film` (
	`film_id` smallint unsigned AUTO_INCREMENT NOT NULL,
...
CREATE TABLE `film_actor` (
	...
	`last_update` timestamp NOT NULL DEFAULT (CURRENT_TIMESTAMP) ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT `film_actor_actor_id_film_id` PRIMARY KEY(`actor_id`,`film_id`)
);
--> statement-breakpoint
CREATE TABLE `film_category` (
	...
	`last_update` timestamp NOT NULL DEFAULT (CURRENT_TIMESTAMP) ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT `film_category_film_id_category_id` PRIMARY KEY(`film_id`,`category_id`)
);
--> statement-breakpoint
CREATE TABLE `film_text` (
	...
	`description` text,
	CONSTRAINT `film_text_film_id` PRIMARY KEY(`film_id`)
);
--> statement-breakpoint
...
CREATE INDEX `idx_location` ON `address` (`location`);--> statement-breakpoint
